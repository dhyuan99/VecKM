...
def main(args):
    ...
    print(f'#parameters: {sum(p.numel() for p in classifier.parameters() if p.requires_grad)}')
    ...
    def weights_init(m):
        ...
        elif classname.find('ComplexLinear') != -1:
            pass
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in enumerate(trainDataLoader):
            ...
            categorical = to_categorical(label, num_classes)

            seg_pred, trans_feat = classifier(points, categorical)
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, label, target) in enumerate(testDataLoader):
                ...
                if epoch == 0 and batch_id > 10 and len(points) == args.batch_size:
                    starter.record()
                    seg_pred, _ = classifier(points, categorical)
                    ender.record()
        ...
        if epoch == 0:
            log_string(f'test total time batch statistics: {total_time}.')
            log_string(f'test total time is mean/std/min/max: {np.mean(total_time)}/{np.std(total_time)}/{np.min(total_time)}/{np.max(total_time)}.')
